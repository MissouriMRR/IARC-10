...
class GimbalPose:  # yaw pitch role values of gimbal----converted to default parameter
    ...
    pitch: float = (
        0  # degrees  # This default parameter assumes that the camera is pointing straight down
    )
...
def meters_to_latlon(
    east, north, latitude
):  # coverts meters into latitude and longitude coordinates
    earth_radius = 6378137.0
    dlat = (
        north / earth_radius
    )  # dlat and dlong are measured in radians; this is the arc length formula s=rtheta
...
def pixel_to_geocoord_gimbal(
    px, py, image_width, image_height, h_fov, v_fov, drone: DronePose, gimbal: GimbalPose
):
    """
    ...
    ray_camera = np.array([x * tan(h_fov / 2), y * tan(v_fov / 2), -1])

    ray_camera /= np.linalg.norm(ray_camera)

    # ---- Rotations ----
    R_drone = rotation_matrix(drone.yaw, drone.pitch, drone.roll)

    R_gimbal = rotation_matrix(gimbal.yaw, gimbal.pitch, gimbal.roll)
    ...
    return (
        lat,
        lon,
    )  # Returns the latitude and longitude of pixel. Drone position plus the change in lat and lon
