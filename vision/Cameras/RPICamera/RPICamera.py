...
from Cameras.Camera_utils.image import Image
from Cameras.Camera_utils.MineDetection import MineDetection
...
class RPICamera(baseCamera):
   ...
   def initialize_camera(self) -> None:
      imx500 = IMX500(self.config["modelPath"])
      ...
      self.input_w, self.input_h = imx500.get_input_size()

      with open(self.config["labelsPath"]) as f:
        self.labels = [line.strip() for line in f if line.strip()]

   def capture_and_detect_mines(self) -> list[MineDetection]:
      ...
      for box, score, cls in zip(boxes, scores, classes):
            ...
            dets.append((name, float(score), box))
            dets.append(MineDetection(float(score), box, (self.input_w, self.input_h)))
