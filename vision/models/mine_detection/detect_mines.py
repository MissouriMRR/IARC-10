...
def detect_mines(
    ...
    for box in mineBoxes:
        tempCoord: Coordinate = Coordinate.from_image_coord(
            box.x_min, box.y_min, box.width, box.height, coord, attitude, camera_info
        )
...
def detect_mines_in_image(image: cv2.typing.MatLike) -> Iterable[BoundingBox]:
    ...
    ref_pic_hist: npt.NDArray[np.float64] = get_lbp_hist(
        ref_pic
    )  # this is what we'll actually use in our comparisons
    ...
    for contour in contours:
        ...
        if (w <= (0.25 * image.shape[1])) and (h <= (0.25 * image.shape[0])) and ((w * h) > 10000):
            ...
            text: str = f"Box {num}"
            cv2.putText(
                copy, text, (x, y), cv2.FONT_HERSHEY_COMPLEX, 3, (255, 255, 255), 2, cv2.LINE_AA
            )
            ...
            if cropped is None:
                ...
                comparison: float = compare_textures(
                    cropped_hist, ref_pic_hist, method="euclidean"
                )  # bhattacharyya is current best
