...
class Minefield:
            ...
            for col in range(len(self.minefield[0])):
                ...
                if self.minefield[row][col] in [Minefield.mineSymbol]:
                    for r in range(1,radius+1):
                        ...
                        if self.minefield[row+downShift][col] in falseMineCriteria or self.minefield[row-upShift][col] in falseMineCriteria:
                            falseMine = True
                        ...
                        # If either direction contains the falseMineCriteria, the current mine becomes
                        # false and cannot be overriden within the main loop over the minefield.
                        if self.minefield[row][col+rightShift] in falseMineCriteria or self.minefield[row][col-leftShift] in falseMineCriteria:
                            falseMine = True
    ...
    def displayOnlyFalseMines(self,print:bool = True):
        ...
        for r,row in enumerate(falseOnlyField):
            for c,col in enumerate(row):
                if falseOnlyField[r][c] in [Minefield.pathSymbol,Minefield.mineSymbol,Minefield.dangerZoneSymbol]:
    ...
    def displayOnlyPaths(self,print:bool = True):
        ...
        for r,row in enumerate(pathsOnlyField):
            for c,col in enumerate(row):
                if pathsOnlyField[r][c] in [Minefield.mineSymbol,Minefield.dangerZoneSymbol,Minefield.falseMineSymbol]:
