...
from flight.pathfinding.nodeField.node_generation import Field
...
class Pathfinder:
    def __init__(
        self, real_corner_coords: tuple[tuple[float, float]], altitude: float, fov_deg: float
    ):

        self.SIM_WIDTH: float = (
            300  # Confirm with nat what this is exactly, this should be an internal constant
        )
