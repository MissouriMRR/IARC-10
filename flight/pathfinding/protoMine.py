...
class protoMine:
    ...
    def __init__(self, safteyRadius, mineLatLon, centerGridOffset):
        ...
        self.blockMatrix = np.zeros(
            (self.placeholderGridSideLength, self.placeholderGridSideLength), dtype=int
        ).tolist()
        self.blockMatrix[self.placeholderGridSideLength // 2][
            self.placeholderGridSideLength // 2
        ] = 1
        self.mineLocation = tuple(
            self.centerOfBlock(
                self.placeholderGridSideLength // 2, self.placeholderGridSideLength // 2
            )
        )
    ...
    def generateBlocks(self):
        ...
        for y in range(len(self.blockMatrix)):
            for x in range(len(self.blockMatrix[y])):
                ...
                self.blockMatrix[y][x] = int(
                    protoMine.circle_rect_intersects(
                        self.mineLocation[0],
                        self.mineLocation[1],
                        self.safteyBlockRadius,
                        xCoord,
                        yCoord,
                        SQUARE_SIDE_LENGTH_FT,
                    )
                )
    ...
    def generateNodes(self):
        ...
        for i in vertexIndicies:
            self.nodeVertices.append(
                BlockMineNode(wrappingVertices[i][0], wrappingVertices[i][1], wrappingDirections[i])
            )
    ...
    def visualize(self):
        ...
        ax.add_patch(
            Circle(
                (self.mineLocation[0], self.mineLocation[1]),
                self.safteyBlockRadius,
                color="green",
                alpha=0.5,
            )
        ...
        poly = Line2D(
            [vertex.x for vertex in self.nodeVertices],
            [vertex.y for vertex in self.nodeVertices],
            color="orangered",
            linewidth=3,
        )
        ...
        for y in range(len(self.blockMatrix)):
            poly = Line2D(
                [0, self.placeholderGridSideLength * SQUARE_SIDE_LENGTH_FT],
                [y * SQUARE_SIDE_LENGTH_FT, y * SQUARE_SIDE_LENGTH_FT],
                color="red",
                linewidth=1,
            )
            ax.add_line(poly)
        for x in range(len(self.blockMatrix[0])):
            poly = Line2D(
                [x * SQUARE_SIDE_LENGTH_FT, x * SQUARE_SIDE_LENGTH_FT],
                [0, self.placeholderGridSideLength * SQUARE_SIDE_LENGTH_FT],
                color="red",
                linewidth=1,
            )
