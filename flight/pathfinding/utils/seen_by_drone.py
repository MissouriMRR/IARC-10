...
class SightTracker:
    ...
    def __init__(
        self, fieldSize: tuple[tuple[int, int], tuple[int, int], tuple[int, int], tuple[int, int]]
    ):
    ...
    def check_holes(
        self, prePhoto: maskGen.PolygonMask, pathSegment: maskGen.PolygonMask
        ...
        for i in range(pathSegment.bottom_x, pathSegment.top_x):
            for j in range(pathSegment.bottom_y, pathSegment.top_y):
                if (
                    self.map[i][j] == 0
                    and prePhoto.body[i - prePhoto.bottom_x][j - prePhoto.bottom_y] > 0
                    and pathSegment.body[i - pathSegment.bottom_x][j - pathSegment.bottom_y] > 0
                ):
                    result = False
        return result
