...
class SightTracker:
    ...
    def note_field_borders(self, field_borders:tuple[tuple[int, int], tuple[int, int], tuple[int, int], tuple[int, int]]):
        temp = [field_borders[0],field_borders[1],field_borders[3],field_borders[2]]
        pic_shape = Image.new("L",self.field_size, 255)
        ImageDraw.Draw(pic_shape).polygon(temp, outline=0, fill=0)
    ...
    def note_pic(self, corner_coords:tuple[tuple[int, int], tuple[int, int], tuple[int, int], tuple[int, int]]):
        temp = [corner_coords[0],corner_coords[1],corner_coords[3],corner_coords[2]]
        picShape = Image.new("L", self.field_size, 0)
        ImageDraw.Draw(picShape).polygon(temp, outline=1, fill=1)
    ...
    def check_holes(self, proto_photo:maskGen.PolygonMask, path_seg:maskGen.PolygonMask): # Maybe include the previous and following path segments?
        for i in range(max(proto_photo.bottom_x, 0), min(proto_photo.top_x, self.field_size[0]), 1):
            for j in range(max(proto_photo.bottom_y, 0), min(proto_photo.top_y, self.field_size[1]), 1):
                print(i)
                print(j)
...
def remove_extra_coords(seen:SightTracker, goto_points:tuple[tuple[float,float]], data_sup:tuple[tuple[pathSub.Node,pathSub.Node,bool]], cam_size:tuple[float,float]):
    ...
    for i in range(len(goto_points)):
        if (data_sup[i][2] == True):
            ...
            seg_mask = maskGen.PolygonMask().create_self_pie(data_sup[i][0], data_sup[i][1])
            tan_angle = np.arctan((goto_points[i][1] - data_sup[i][0].parentMine.y) / (goto_points[i][0] - data_sup[i][0].parentMine.x)) + (np.pi/2)
            proto_photo = maskGen.PolygonMask().create_self_rect(goto_points[i], tan_angle, cam_size)
        else:
            # Segment is a straight type
            seg_mask = maskGen.PolygonMask().create_self_straight(data_sup[i][0], data_sup[i][1])
...
def remove_extra_coords(seen:SightTracker, goto_points:tuple[tuple[float,float]], data_sup:tuple[tuple[pathSub.Node,pathSub.Node,bool]], cam_size:tuple[float,float], test:bool):
    ...
    seg_image = Image.new("RGB", seen.field_size)
    predict_pic_image = Image.new("RGB", seen.field_size)
    ...
    for i in range(len(goto_points)):
        ...
        temp_array[proto_photo.body] = next_color[(i % 3)]
        temp_image = Image.fromarray(temp_array)
        predict_pic_image.paste(temp_image, (proto_photo.bottom_x, proto_photo.top_y))
...
if __name__ == "__main__":
    ...
    picture_borders = [[50,150],[200,170],[45,85],[195,90]]
    ...
    remove_extra_coords(seen_tracker, goto_points, data_sup, [path.ground_covered_image(64, 20), path.ground_covered_image(64, 20)], True)