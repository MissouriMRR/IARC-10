...
class SightTracker:
    ...
    def note_pic(
        self,
        corner_coords: tuple[tuple[int, int], tuple[int, int], tuple[int, int], tuple[int, int]],
    ):
    ...
    def check_holes(
        ...
        for i in range(max(proto_photo.bottom_x, 0), min(proto_photo.top_x, self.field_size[0]), 1):
            for j in range(
                max(proto_photo.bottom_y, 0), min(proto_photo.top_y, self.field_size[1]), 1
            ):
                print(i)
                print(j)
                if (
...
def remove_extra_coords(
    ...
    for i in range(len(goto_points)):
        if data_sup[i][2] == True:
            ...
            proto_photo = maskGen.PolygonMask().create_self_rect(
                goto_points[i], tan_angle, cam_size
            )
        else:
...
if __name__ == "__main__":
    ...
    remove_extra_coords(
        ...
        goto_points,
        data_sup,
        [path.ground_covered_image(64, 20), path.ground_covered_image(64, 20)],
        True,
    )
