...
class SimToLatLonTransformer:
    def __init__(self, corner_coords: tuple[tuple[float, float]], sim_width: float):
        ...
        self.origin_localx, self.origin_localy = self.latlon_to_local(
...
def run_tests():
    ...
    print(test_point)
    print(
        coord_converter.local_to_latlon(*coord_converter.latlon_to_local(*test_point))
    )  # Should match line above
    print()
    ...
    print(
        coord_converter2.latlon_to_local(*lat_lon3)
    )  # Should be close to (0, 0), x coordinate should be positive
    print(coord_converter2.latlon_to_local(*lat_lon4))  # Should be close to (360, 0)
    print(test_point)
    print(
        coord_converter2.local_to_latlon(*coord_converter2.latlon_to_local(*test_point))
    )  # Should match line above
    print()
    ...
    test_3_local_dist12 = math.sqrt(
        (test_3_point1[0] - test_3_point2[0]) ** 2 + (test_3_point1[1] - test_3_point2[1]) ** 2
    )
    test_3_local_dist23 = math.sqrt(
        (test_3_point2[0] - test_3_point3[0]) ** 2 + (test_3_point2[1] - test_3_point3[1]) ** 2
    )
    test_3_local_dist13 = math.sqrt(
        (test_3_point1[0] - test_3_point3[0]) ** 2 + (test_3_point1[1] - test_3_point3[1]) ** 2
    )
    ...
    test_4_local_dist12 = math.sqrt(
        (test_4_point1[0] - test_4_point2[0]) ** 2 + (test_4_point1[1] - test_4_point2[1]) ** 2
    )
    test_4_local_dist23 = math.sqrt(
        (test_4_point2[0] - test_4_point3[0]) ** 2 + (test_4_point2[1] - test_4_point3[1]) ** 2
    )
    test_4_local_dist13 = math.sqrt(
        (test_4_point1[0] - test_4_point3[0]) ** 2 + (test_4_point1[1] - test_4_point3[1]) ** 2
    )
