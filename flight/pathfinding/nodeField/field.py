...
from ...newPathfinding import diamondMine as dM
...
class Field:
    ...
    def addMine(self, centerX: float, centerY: float, radius: int, color: str = ""):
        ...
        for pair in mineCombo:
            ...
            mine.mineDistances[target] = np.sqrt(
                (mine.x - target.x) ** 2 + (mine.y - target.y) ** 2
            )
            target.mineDistances[mine] = mine.mineDistances[target]

        for fNode in self.floatingNodes:
            mineNodePrimary = MineNode(
                parentMine=newMine, floatingNode=fNode, primary=True, connectedToFloating=True
            )
            mineNodeSecondary = MineNode(
                parentMine=newMine, floatingNode=fNode, primary=False, connectedToFloating=True
            )
