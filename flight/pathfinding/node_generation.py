...
class Connection:
    ...
    def validHuggingEdge(self, intersectionAngle, offsetAngle) -> bool:
        ...
        intersectionPointAngle1, intersectionPointAngle2 = (
            intersectionAngle + offsetAngle,
            intersectionAngle - offsetAngle,
        )
        ...
        if firstNodeAngle > secondNodeAngle:
            if (
                ...
                and intersectionPointAngle1 > firstNodeAngle - np.pi * 2
            ):
    ...
    def generateIntersectionPoints(mine1: "Mine", mine2: "Mine") -> list[float]:
        ...
        radicalLineDistance: float = ((mine1.radius**2 - mine2.radius**2 + distance**2)) / (
            2 * distance
        )
        ...
        intersectP1 = [
            mine1.radius * np.cos(intersectionAngle + offsetAngle) + mine1.x,
            mine1.radius * np.sin(intersectionAngle + offsetAngle) + mine1.y,
        ]
        intersectP2 = [
            mine1.radius * np.cos(-intersectionAngle + offsetAngle) + mine1.x,
            mine1.radius * np.sin(-intersectionAngle + offsetAngle) + mine1.y,
...
class Field:
    ...
    def addFloatingNode(self, x: int, y: int) -> "Node":
        ...
        for mine in Connection.field.mines:
            ...
            mineNodeSecondary = MineNode(
                parentMine=mine, floatingNode=fNode, primary=False, connectedToFloating=True
            )
    ...
    def plotField(
        ...
        labeled: bool = False,
        path=[],
        title: str = "Mines and Potential Paths",
        xlabel: str = "",
    ) -> None:
        plt = pyplot
        fig, ax = plt.subplots()
        ...
        circles = [
            plt.Circle(Mine.mines[i].getPos(), Mine.mines[i].getRadius(), color=Mine.mines[i].color)
            for i in range(len(Mine.mines))
        ]
